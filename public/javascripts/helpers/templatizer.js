    ...
    return (
    {
        ...
        init: function()
        {
            ...
            $('.template').each(oHelpers.createCallback(this, function(i, eTemplate)
            {
                ...
                var sText = $(eTemplate).text();
                this.compileTemplate(sName, sText);
            }));
        },
        ...
        compileTemplate: function(sName, sText)
        {
            try
            {
                this._templates[sName] = this._compileTemplate(sName, sText);    
            }
            catch(oError)
            {
                if (oError == 'TEMPLATE COMPILE ERROR')
                    throw('templates "' + sName + '" failed to compile.\n\nSee the console log for details.');
                else
                    throw oError;
            }
        },
        ...
        _compileTemplate: function(sName, sTemplate)
        {
            ...
            return {
                ...
                {
                    return fnRender(oData, aParts, this._escapeHtml, this);
                })
            }
        },
